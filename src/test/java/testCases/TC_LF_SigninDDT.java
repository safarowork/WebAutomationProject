...
public class TC_LF_SigninDDT extends BaseClass{
	HomePage homepage;
	...
	public void testSigninWithValidCredentials(String email, String password, String status) {
		...
		if(status.equals("invalidemail")) {
			if(signuppage.validateIncorrectSiginPasswordOrEmail()==true)
				...
				Assert.fail();
			else if(homepage.validateLogoutDisplay()==true) {
		...
		if(status.equals("valid")) {
			if(homepage.validateLogoutDisplay()==true) {
				Assert.assertTrue(true);
				homepage.clickLogOut();
			}
			else if(homepage.validateLogoutDisplay()==false)
				Assert.fail();
		}
